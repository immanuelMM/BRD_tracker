...
const fmtMeetingDateTime = (m) => {
  if (!m.date) return m.time || 'No date/time set';
  ...
  return m.time ? `${dateStr} · ${d.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}` : dateStr;
};
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`rounded-2xl border-2 p-6 ${isSuccess ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
        ...
        {/* Meetings */}
        {parseMeetings(brd.meetings).length > 0 && (
          <div className="mt-3 bg-teal-50 dark:bg-teal-950/30 border border-teal-100 dark:border-teal-800 rounded-xl px-4 py-3">
            <p className="text-xs font-semibold text-teal-600 dark:text-teal-400 uppercase tracking-wide mb-2">Meetings</p>
            <div className="space-y-2">
              {parseMeetings(brd.meetings).map((meeting, idx) => (
                <div key={idx} className="p-2.5 bg-white dark:bg-slate-800 rounded-lg">
                  <div className="flex items-center gap-2">
                    <svg className="w-3.5 h-3.5 text-teal-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                    <p className="text-sm font-medium text-slate-900 dark:text-white">Meeting {idx + 1} — {fmtMeetingDateTime(meeting)}</p>
                  </div>
                  {(meeting.links || []).length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-2 pl-5">
                      {meeting.links.map((url, linkIdx) => (
                        <a key={linkIdx} href={url} target="_blank" rel="noopener noreferrer"
                          ...
                          Link {meeting.links.length > 1 ? linkIdx + 1 : ''}
                        </a>
                      ))}
                    </div>
                  )}
