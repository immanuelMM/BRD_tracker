...
export default function BRDForm({ initial = {}, onSave, onCancel, teamLeads = [] }) {
  const [form, setForm] = useState({
    ...
    meetings: parseMeetings(initial.meetings),
  });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!validate()) return;
    const { extendedQuarters, beTickets, feTickets, anciliaryTickets, rndTickets, meetings, ...rest } = form;
    const cleanedMeetings = meetings
      .map((m) => ({ ...m, links: (m.links || []).filter(Boolean) }))
      .filter((m) => m.date || m.time || m.links.length > 0);
    onSave({
      ...rest,
      ...
      meetings:        cleanedMeetings.length > 0 ? JSON.stringify(cleanedMeetings) : null,
    });
  };
  ...
  // Meeting handlers
  const addMeeting = () => setForm((f) => ({ ...f, meetings: [...f.meetings, { date: '', time: '', links: [''] }] }));
  const removeMeeting = (idx) => setForm((f) => ({ ...f, meetings: f.meetings.filter((_, i) => i !== idx) }));
  const setMeetingField = (idx, field, val) => setForm((f) => ({ ...f, meetings: f.meetings.map((m, i) => i === idx ? { ...m, [field]: val } : m) }));
  const addMeetingLink = (idx) => setForm((f) => ({ ...f, meetings: f.meetings.map((m, i) => i === idx ? { ...m, links: [...(m.links || []), ''] } : m) }));
  const removeMeetingLink = (idx, linkIdx) => setForm((f) => ({ ...f, meetings: f.meetings.map((m, i) => i === idx ? { ...m, links: m.links.filter((_, li) => li !== linkIdx) } : m) }));
  const setMeetingLink = (idx, linkIdx, val) => setForm((f) => ({ ...f, meetings: f.meetings.map((m, i) => i === idx ? { ...m, links: m.links.map((l, li) => li === linkIdx ? val : l) } : m) }));
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl space-y-3">
          ...
          {form.meetings.length === 0 && (
            <p className="text-xs text-slate-400">No meetings added yet</p>
          )}

          <div className="space-y-3">
            {form.meetings.map((meeting, idx) => (
              <div key={idx} className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wide">Meeting {idx + 1}</span>
                  <button type="button" onClick={() => removeMeeting(idx)}
                    className="w-6 h-6 flex items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-700 hover:bg-red-100 dark:hover:bg-red-950 text-slate-400 hover:text-red-500 transition-colors text-sm leading-none">
                    ×
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <input type="date" value={meeting.date || ''} onChange={(e) => setMeetingField(idx, 'date', e.target.value)} className={inputClass} />
                  <input type="time" value={meeting.time || ''} onChange={(e) => setMeetingField(idx, 'time', e.target.value)} className={inputClass} />
                </div>
                <div className="space-y-1.5">
                  {(meeting.links || []).map((url, linkIdx) => (
                    <div key={linkIdx} className="flex gap-1.5">
                      <input
                        value={url}
                        onChange={(e) => setMeetingLink(idx, linkIdx, e.target.value)}
                        placeholder="https://... meeting link (Zoom, Teams, Meet...)"
                        className={inputClass}
                      />
                      <button type="button" onClick={() => removeMeetingLink(idx, linkIdx)}
                        className="flex-shrink-0 w-8 h-[42px] flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-700 hover:bg-red-100 dark:hover:bg-red-950 text-slate-400 hover:text-red-500 transition-colors text-lg leading-none">
                        ×
                      </button>
                    </div>
                  ))}
                  <button type="button" onClick={() => addMeetingLink(idx)}
                    className="w-full py-1.5 rounded-xl text-xs font-semibold transition-colors border border-dashed border-current/30 bg-teal-50 dark:bg-teal-950 text-teal-600 hover:bg-teal-100 dark:hover:bg-teal-900">
                    + Add Link
                  </button>
                </div>
              </div>
          ...
          <button type="button" onClick={addMeeting}
            className="w-full py-1.5 rounded-xl text-xs font-semibold transition-colors border border-dashed border-current/30 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600">
            + Add Meeting
          </button>
        </div>
